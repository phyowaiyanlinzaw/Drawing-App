...
class DrawingView(context : Context, attrs : AttributeSet) : View(context,attrs) {
    ...
    private var mCanvasBitmaps : Bitmap? = null
    ...
    override fun onSizeChanged(w: Int, h: Int, wprev: Int, hprev: Int) {
        ...
        mCanvasBitmaps = Bitmap.createBitmap(w, h, Bitmap.Config.ARGB_8888)
        canvas = Canvas(mCanvasBitmaps!!)
    }

    override fun onDraw(canvas: Canvas) {
        super.onDraw(canvas)
        canvas.drawBitmap(mCanvasBitmaps!!,0f,0f,mCanvasPaint)
        for (path in mPaths){
